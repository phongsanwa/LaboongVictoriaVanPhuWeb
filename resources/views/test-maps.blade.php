...
<div class="test-block">
    ...
    <div id="fix-auth" class="fix-hint">
        ...
        1. Key bị IP-restricted (chỉ dùng cho server, không dùng cho browser).<br>
        ...
        4. Billing chưa được kích hoạt trên Google Cloud project.
    </div>
</div>
...
{{-- Test 3: Geocoding --}}
<div class="test-block">
    <h3>Test 3: Geocoding (google.maps.Geocoder)</h3>
...
<div class="test-block">
    <h3>Test 4: AutocompleteService (cách app dùng để gợi ý)</h3>
...
<script>
    ...
    if (isLocal) {
        ...
        urlEl.textContent = "⚠️ Đang test trên localhost — website restriction không áp dụng ở đây";
    } else {
        ...
        new google.maps.Geocoder().geocode(
            ...
            (results, status) => {
                if (status === "OK" && results?.length) {
                    ...
                    const out = { address: results[0].formatted_address, lat: loc.lat(), lng: loc.lng() };
